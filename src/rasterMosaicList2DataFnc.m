...
%
%   rasterNanFloors =   [j x 1] array indicating the floors of all real
...
%                       routine. If only a single rasterNanFloor value is
%                       provided then that value will, by default, be
%                       applied to all raster datasets encountered.
%
...
parse(P,nargin,nargout,rasterMosaicList,rasterNanFloors,gridMask, ...
    gridMaskGeoRasterRef);
...
for i = 1:listSize
        ...
        if numel(rasterNanFloors) == 1
            
            currentRasterNanFloor = rasterNanFloors;
            
        else
            
            currentRasterNanFloor = rasterNanFloors(i,1);
        ...
        currentRaster(currentRaster <= currentRasterNanFloor) = nan;
